...
def client(monkeypatch):
    """A fresh app against a throwaway database and upload directory."""
    ...
    for module in list(m for m in list(__import__("sys").modules) if m.startswith("app")):
    ...
    with TestClient(app) as c:
        c.data_dir = Path(tmp)  # type: ignore[attr-defined]
        yield c
